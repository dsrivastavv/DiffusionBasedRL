import os
import copy
from diffuser.models.dit import DiT
from diffuser.models.gaussian_diffusion import ImprovedGaussianDiffusion
import numpy as np
import torch
import einops
import pdb
...
class ImprovedDiffusionTrainer(Trainer):
    def __init__(
        self,
    ...
    def train(self, n_train_steps):
        ...
        for step in range(n_train_steps):
            for i in range(self.gradient_accumulate_every):
                ...
                batch = batch_to_device(batch)
                t = torch.randint(
                    0, self.diffusion.num_timesteps, (batch[0].shape[0],), device=self.device
                )
                loss = self.diffusion.training_losses(self.model, batch[0], t)['loss'].mean()
                loss = loss / self.gradient_accumulate_every
                loss.backward()
    ...
    def render_samples(self, batch_size=2, n_samples=2):
        ...
        for i in range(batch_size):
            ...
            samples = self.diffusion.p_sample_loop(self.ema_model, shape, conditions, clip_denoised=False)
